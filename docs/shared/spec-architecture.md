...
*   `docs/`: [[shared/discovery/infraestrutura.ambientes|Gestão de Ambientes (DEV vs PROD)]] e diretrizes de infraestrutura Supabase.
*   `docs/`: [[shared/discovery/sistema.map.relatorios|Manuais técnicos de cálculos]], modelagem de domínio e nutrição.
*   `docs/`: [[features/ingredientes/discovery/sistema.calc.rotulagem|Espec. Técnica de Rotulagem]] e regras de conformidade.
