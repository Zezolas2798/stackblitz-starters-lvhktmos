...
| :--- | :--- | :--- |
| [[features/industria/discovery/fichas_tecnicas_industrial]] | `queries` | A OP consome dados de rendimento e composição das fichas técnicas. |
| [[features/ingredientes/discovery/ingredientes]] | `queries` | A requisição de materiais referencia a base de ingredientes. |
| `app/producao/page.tsx` | `implements` | Página principal do dashboard de produção. |
| [[registry]] | `derives-from` | Registra as entidades `producao.OrdemProducao` e `producao.StatusOP`. |
...
| **Setor de Produção ([[features/industria/discovery/setores_producao]])** | `Entity` | Divisão física/lógica da fábrica (ex: Confeitaria). |
...
- Os setores são isolados por `unidade_id` (ver [[features/industria/discovery/setores_producao]]).

### 5.2. Cálculo de Requisição (`calculates` Requisicao)
A partir da composição da [[features/industria/discovery/fichas_tecnicas_industrial]], o sistema calcula:
