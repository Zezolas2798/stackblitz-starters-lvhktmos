...
import {
  Save, ArrowLeft, Thermometer, Scale, FileText, MapPin, Plus, History, PackageCheck, AlertTriangle, CheckCircle, Printer, Eye
} from 'lucide-react';
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        quantidade_atual_g_ml: qtdReal,
        status: statusFinal as 'QUARENTENA' | 'APROVADO' | 'REJEITADO' | 'VENCIDO'
      };

      const { data: lote, error: erroLote } = await supabase.from('lotes_estoque').insert(payload).select().single();
