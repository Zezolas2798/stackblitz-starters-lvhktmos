...
export default function EstoquePage() {
  ...
  useEffect(() => {
    if (activeClientId) {
      if (activeTab === 0) loadEstoque();
      if (activeTab === 1) loadDescartes();
      if (activeTab === 2) loadReservas();
      loadUnidadeInfo();
    }
  }, [activeClientId, activeTab, unidadeId]);
  ...
  async function loadEstoque() {
    ...
    const { data: lotesData, error } = await (supabase as any)
      .from('lotes_estoque')
      .select('*, ingredientes(nome, grupo_estoque_id, ingredientes_grupos(nome)), fornecedores(razao_social, nome_fantasia, cnpj), cliente_locais_estoque(nome)')
    ...
    if (lotesData) {
      ...
      const locaisDb = locaisData ? locaisData.map((l: any) => l.nome) : [];
      const categoriasDb = catsData ? catsData.map((c: any) => c.nome) : [];

      setLocaisDisponiveis(locaisDb.length > 0 ? locaisDb : ['Nenhum local cadastrado']);
      setCategoriasDisponiveis(categoriasDb.length > 0 ? categoriasDb : ['Geral', 'Secos', 'Refrigerados', 'Congelados']);
    }
    setLoading(false);
  }
  ...
  async function loadDescartes() {
    ...
    const { data: descData, error: descErr } = await (supabase as any)
      .from('estoque_movimentacoes')
      .select('*, lotes_estoque(*, ingredientes(nome), fornecedores(razao_social))')
      .eq('tipo_movimento', 'SAIDA')
      .ilike('justificativa', '%DESCARTADO%')
      .order('data_movimento', { ascending: false });

    // Busca todas as categorias de produto do cliente para o filtro de Descartes
    const { data: catsData } = await (supabase as any)
      .from('cliente_categorias_produto')
      .select('nome')
      .eq('cliente_id', activeClientId!);
    ...
    if (descData) {
      setDescartes(descData);

      const catsDb = catsData ? catsData.map((c: any) => c.nome) : [];
      setCategoriasDescDisponiveis(catsDb.length > 0 ? catsDb : ['Geral', 'Secos', 'Refrigerados', 'Congelados']);
    }
    setDescartesLoading(false);
  }
  ...
  async function loadReservas() {
    ...
    try {
      ...
      const { data: stockLevels } = await (supabase as any)
        .from('lotes_estoque')
        .select('quantidade_atual_g_ml, ingrediente_id, ingredientes(grupo_estoque_id)')
        .eq('unidade_id', unidadeId)
        .neq('status', 'REJEITADO')
        .neq('status', 'PREVISTO')
        .is('deleted_at', null);
      ...
      const stockBalanceMap: { [key: string]: number } = {};
      ...
      (reqData || []).forEach((req: any) => {
        ...
        req.saldo_estoque_live = stockBalanceMap[key] || 0;
  ...
  const formatarQuantidade = (volumeGml: number) => {
    if (volumeGml >= 1000) {
      return `${(volumeGml / 1000).toLocaleString('pt-BR', { maximumFractionDigits: 2 })} Kg/L`;
    }
    return `${volumeGml} g/ml`;
  };
  ...
  const lotesFiltrados = lotes.filter(lote => {
    ...
    // Check if it belongs to a group, otherwise use its own name
    const nomeIngredienteOuGrupo = lote.ingredientes?.ingredientes_grupos?.nome 
        || lote.ingredientes?.nome 
        || '';

    const matchBusca =
      nomeIngredienteOuGrupo.toLowerCase().includes(termo) ||
      (lote.ingredientes?.nome || '').toLowerCase().includes(termo) || // Still allow searching by specific brand
  ...
  const descartesFiltrados = descartes.filter(desc => {
    const nomeProduto = desc.lotes_estoque?.ingredientes?.nome || '';
    const nomeFornecedor = desc.lotes_estoque?.fornecedores?.razao_social || '';
    const loteProduto = desc.lotes_estoque?.numero_lote_fabricante || '';
    const categoria = desc.lotes_estoque?.categoria_produto || '';
  ...
  const groupedLotes = useMemo(() => {
    const grupos: Record<string, { tituloGrupo: string, lotes: any[], totalGml: number }> = {};
    
    lotesFiltrados.forEach(lote => {
      // Determina o nome do grupo agregador
      const tituloGrupo = lote.ingredientes?.ingredientes_grupos?.nome || lote.ingredientes?.nome || 'Produto Desconhecido';
      
      if (!grupos[tituloGrupo]) {
        grupos[tituloGrupo] = { tituloGrupo, lotes: [], totalGml: 0 };
      }
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 10 }}>
      ...
      >
        ...
        <Tab icon={<Trash2 size={18} />} iconPosition="start" label="Análise de Descarte" />
        <Tab icon={<Truck size={18} />} iconPosition="start" label="Reservas (Produção)" />
      </Tabs>
                          ...
                          <TableRow hover sx={{ '&:hover': { bgcolor: alpha(theme.palette.primary.main, 0.04) }, cursor: 'pointer' }} onClick={() => toggleGroup(key)}>
                            ...
                            <TableCell>
                              <Typography variant="h6" color="primary.main" sx={{ fontWeight: 800 }}>
                                {formatarQuantidade(grupo.totalGml)}
                              </Typography>
                            </TableCell>
                              ...
                              <TableCell>
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, alignItems: 'flex-start' }}>
                                  <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>
                                    {formatarQuantidade(lote.quantidade_atual_g_ml - (lote.total_reservado_g || 0))} 
                                  ...
                                  {(lote.total_reservado_g || 0) > 0 && (
                                    <Tooltip title={
                                      ...
                                      <Chip 
                                        label={`${formatarQuantidade(lote.total_reservado_g)} Reserv.`} 
                                        size="small" 
                                        color="warning" 
                                        variant="outlined"
                      ...
                      <TableRow key={desc.id} hover>
                        ...
                        <TableCell>
                          <Typography variant="body2" color="error.main" fontWeight={700}>
                            {formatarQuantidade(desc.quantidade_movimentada)}
                          </Typography>
                        </TableCell>
                        <TableCell>
                              ...
                              <React.Fragment key={req.id}>
                                ...
                                <TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.01) }}>
                                  <TableCell>
                                    ...
                                    <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                                      <Typography variant="caption" color="text.secondary">Nec.: {formatarQuantidade(req.qtd_necessaria_g)}</Typography>
                                      <Typography variant="caption" sx={{ 
                                        ...
                                        Saldo em Estoque: {formatarQuantidade(req.saldo_estoque_live || 0)}
                                      </Typography>
                                    </Box>
                                  </TableCell>
                                  <TableCell colSpan={2}>
                                    {(req.saldo_estoque_live || 0) <= 0 && req.qtd_separada_g < req.qtd_necessaria_g ? (
                                      ...
                                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                        <Typography variant="caption" color="warning.dark" sx={{ fontStyle: 'italic', fontWeight: 600 }}>
                                          Pendente Separar: {formatarQuantidade(req.qtd_necessaria_g - req.qtd_separada_g)}
                                        </Typography>
                                ...
                                {reservas.map((res: any) => (
                                  <TableRow key={res.id}>
                                    ...
                                    <TableCell align="right">
                                      <Typography variant="body2" fontWeight="bold">{formatarQuantidade(res.quantidade_reservada_g)}</Typography>
                                    </TableCell>
                                    <TableCell align="center">
             ...
             <Grid container spacing={2}>
               <Grid item xs={6}>
                 ...
                 <Typography variant="h6" fontWeight="bold" color="primary.main">
                   {reservaDialog ? formatarQuantidade(reservaDialog.qtd_necessaria_g - reservaDialog.qtd_separada_g) : '0'}
                 </Typography>
               </Grid>
