...
export default function DashboardQualidadePage() {
    ...
    const handleDelete = async (id: string) => {
        // Confirmação com menção à Trilha de Auditoria (Soft Delete)
        if (!confirm('Tem certeza que deseja inativar esta auditoria? O laudo será ocultado dos relatórios oficiais (preservando o histórico interno GxP).')) {
            return;
        }

        try {
            // Busca Sessão do Funcionário para RLS
            const { data: { session } } = await supabase.auth.getSession();

            // Endpoint Seguro de Inativação Qualidade (Zod)
            const response = await fetch('/api/qualidade/auditorias', {
                method: 'DELETE',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errData.erro || errData.dbDetalhes || 'Falha ao inativar laudo.');
            }

            // Atualiza a lista visualmente removendo o item ocultado
