  ...
  if (!ordem || error) {
    return (
      <Container maxWidth="xl" sx={{ mt: 4 }}>
        ...
        <Button startIcon={<ArrowLeft size={18} />} onClick={() => router.push('/producao')} sx={{ mt: 2 }}>
          Voltar para Lista
        </Button>
      </Container>
    );
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 12 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 4, gap: 2 }}>
        <IconButton onClick={() => router.push('/producao')} sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider' }}>
          <ArrowLeft size={20} />
        </IconButton>
        <Box sx={{ flex: 1 }}>
