    ...
    try {
      ...
      for (const item of composicao) {
        ...
        const { data: lotes } = await supabase
          .from('estoque_lotes')
          .select('id, codigo_lote_fornecedor, data_validade_atual, quantidade_atual')
          .eq('ingrediente_id', item.item_id)
          .gt('quantidade_atual', 0)
          .gte('data_validade_atual', new Date().toISOString()) 
        ...
        if (lotes) {
          for (const lote of lotes) {
            ...
            const estoqueEmGramas = Number(lote.quantidade_atual) * fator;
            ...
            lotesSugeridos.push({
              lote_id: lote.id,
              codigo: lote.codigo_lote_fornecedor,
              validade: lote.data_validade_atual,
  ...
  async function handleFinalizar() {
    ...
    try {
      ...
      const { error } = await supabase.rpc('registrar_producao', {
        ...
        p_responsavel_id: userData.user?.id || null,
