...
import { 
  ...
  CircularProgress, Switch, ToggleButton, ToggleButtonGroup
} from '@mui/material';
...
export default function NovoIngredientePage() {
    ...
    if (isCientifica) {
      const loadReferencias = async () => {
        ...
        try {
          ...
          console.log(`[Novo Ingrediente] Referências ${fonteCientifica}: ${allData.length}`);
  ...
  // Lógica Aditivos e Alérgenos (Mesma do anterior)
  ...
  if (!activeClientId && !loading) {
    return (
        <Box p={4}>
            <Alert severity="warning">Selecione uma unidade no menu lateral para cadastrar ingredientes.</Alert>
        </Box>
    );
  }
  ...
  return (
    <Box>
      <Box sx={{ mb: 3, display: 'flex', alignItems: 'center' }}>
        ...
        <Box>
            <Typography variant="h5" fontWeight="bold">Novo Ingrediente</Typography>
      ...
      <Paper sx={{ width: '100%', borderRadius: 2, overflow: 'hidden', border: '1px solid', borderColor: 'divider' }}>
        ...
        <Box sx={{ p: 4 }}>
          ...
          {tabIndex === 0 && (
            <Grid container spacing={3}>
              <Grid item xs={12} md={4}>
                <TextField select label="Tipo de Classificação" fullWidth value={formData.tipo_ingrediente} onChange={e => handleChange('tipo_ingrediente', e.target.value)}>
          ...
          >
            {loading ? 'Salvando...' : 'Cadastrar Ingrediente Completo'}
          </Button>
        </Box>
      </Paper>
