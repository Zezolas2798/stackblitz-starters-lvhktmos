...
import { 
  ...
  TextField, Box, Tabs, Tab, InputAdornment, 
...
interface QuickIngredienteDialogProps {
  open: boolean;
  onClose: () => void;
  onSuccess: (novoIngrediente: any, categoriaSugerida?: string) => void;
  nomeSugerido?: string;
}

export default function QuickIngredienteDialog({ open, onClose, onSuccess, nomeSugerido }: QuickIngredienteDialogProps) {
  ...
  }, [open, nomeSugerido, activeClientId]);

  const loadDadosAuxiliares = async () => {
    const { data: alergData } = await supabase.from('anvisa_alergenicos').select('*').order('nome');
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (categoriaValue) {
        if (typeof categoriaValue === 'object' && !categoriaValue.inputValue) nomeCategoriaFinal = categoriaValue.nome;
        ...
        if (nomeCategoriaFinal) {
            const existe = categoriasMestre.some(c => c.nome.toLowerCase() === nomeCategoriaFinal.toLowerCase());
            if (!existe) await supabase.from('cliente_categorias_produto').insert({ cliente_id: activeClientId, nome: nomeCategoriaFinal });
        }
      }
      ...
      const { data: ing, error: errIng } = await supabase.from('ingredientes').insert(payloadIngrediente).select().single();

      if (errIng) throw errIng;

      // 3. Alergênicos
      if (alergenicosSelecionados.length > 0) {
        const links = alergenicosSelecionados.map(a => ({ ingrediente_id: ing.id, alergenico_id: a.id, contem: true, contem_derivado: false }));
        await supabase.from('ingredientes_alergenicos_link').insert(links);
      }
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: 'grey.50' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="h6" fontWeight="bold">Cadastro Mestre de Ingrediente</Typography>
        </Box>
