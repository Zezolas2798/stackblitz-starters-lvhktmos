...
// Tipos Locais para o Formul├írio
interface ItemForm {
    id?: string;
    tempId: string;
...
function EditorModeloChecklistContent() {
    ...
    // Carregar dados se for edi├º├úo
    useEffect(() => {
        if (editingId && activeClientId) {
            loadModeloCompleto(editingId);
        } else {
            // Se for novo, inicia com uma se├º├úo vazia
            handleAddSecao();
        }
    ...
    const handleRemoveSecao = (index: number) => {
        if (!confirm('Remover esta se├º├úo apagar├í todas as perguntas dentro dela. Continuar?')) return;
        const novas = [...secoes];
    ...
    // --- MANIPULA├ç├âO DE ITENS (PERGUNTAS) ---
    ...
    const handleSave = async () => {
        ...
        if (secoes.length === 0) return alert('Adicione pelo menos uma se├º├úo.');

        for (const secao of secoes) {
            if (!secao.titulo.trim()) return alert('Todas as se├º├Áes precisam de um t├¡tulo.');
            if (secao.itens.length === 0) return alert(`A se├º├úo "${secao.titulo}" est├í vazia.`);
            for (const item of secao.itens) {
                if (!item.texto_pergunta.trim()) return alert(`Existem perguntas sem texto na se├º├úo "${secao.titulo}".`);
            }
        }
        ...
        try {
            ...
            // 2. Limpar estrutura antiga (Simples e eficaz para edi├º├úo)
            if (editingId) {
            ...
            for (let i = 0; i < secoes.length; i++) {
                ...
                if (secao.itens.length > 0) {
                    const itensPayload = secao.itens.map((item, idx) => ({
                        ...
                        modelo_id: modeloId, // Mantemos redund├óncia para facilitar queries se necess├írio
    ...
    return (
        <Container maxWidth="md" sx={{ mt: 4, mb: 12 }}>
            ...
            <Paper elevation={0} sx={{ p: 3, mb: 4, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
                <Typography variant="h6" fontWeight="bold" gutterBottom color="primary.main">Defini├º├Áes do Formul├írio</Typography>
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={4}>
                        <FormControl fullWidth>
                            <InputLabel shrink>Frequ├¬ncia Sugerida</InputLabel>
                            <Select value={frequencia} label="Frequ├¬ncia Sugerida" onChange={e => setFrequencia(e.target.value)}>
                                <MenuItem value="DIARIO">Di├írio (Rotina)</MenuItem>
                    ...
                    <Grid item xs={12}>
                        <TextField
                            label="Instru├º├Áes Gerais para o Operador" fullWidth multiline rows={2}
                            value={descricao} onChange={e => setDescricao(e.target.value)}
                            placeholder="Ex: Preencher antes de iniciar a produ├º├úo. Notificar gerente se houver inconformidade." InputLabelProps={{ shrink: true }}
                        />
                    </Grid>
                </Grid>
            </Paper>

            {/* CONSTRUTOR DE SE├ç├òES */}
            <Box sx={{ mb: 4 }}>
                    ...
                    <Accordion key={secao.tempId} defaultExpanded sx={{ mb: 3, border: '1px solid #e0e0e0', borderRadius: '8px !important', '&:before': { display: 'none' }, boxShadow: '0 2px 8px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                        <AccordionSummary expandIcon={<ChevronDown />} sx={{ bgcolor: alpha(theme.palette.primary.main, 0.05), borderBottom: '1px solid #eee' }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 2 }} onClick={e => e.stopPropagation()}>
                                ...
                                <TextField
                                    placeholder="Nome da Se├º├úo (Ex: Geladeiras...)" variant="standard" fullWidth
                                ...
                                />
                                <Tooltip title="Remover Se├º├úo"><IconButton size="small" color="error" onClick={() => handleRemoveSecao(sIdx)}><Trash2 size={18} /></IconButton></Tooltip>
                            </Box>
                        </AccordionSummary>
                        ...
                        <AccordionDetails sx={{ p: 0, bgcolor: 'background.paper' }}>
                            <Box sx={{ p: 3 }}>
                                    ...
                                    <Box key={item.tempId} sx={{ display: 'flex', flexDirection: 'column', gap: 1, p: 2, mb: 2, border: '1px solid #eee', borderRadius: 2, '&:hover': { borderColor: theme.palette.primary.light, bgcolor: alpha(theme.palette.primary.main, 0.01) } }}>
                                            ...
                                            <Grid item xs={12} md={4}>
                                                <FormControl fullWidth size="small">
                                                    ...
                                                    <Select value={item.tipo_resposta} label="Tipo de Resposta" onChange={e => handleUpdateItem(sIdx, iIdx, 'tipo_resposta', e.target.value)}>
                                                        {/* OP├ç├âO ATUALIZADA COM N.A. */}
                                                        <MenuItem value="CONFORME_NAOCONFORME"><Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}><CheckSquare size={16} className="text-green-600" /> Conforme / N├úo Conforme / N.A.</Box></MenuItem>
                                                        ...
                                                        <MenuItem value="NUMERO"><Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}><Hash size={16} className="text-orange-600" /> Num├®rico (Qtd/Peso)</Box></MenuItem>
                                        ...
                                        <Grid container spacing={2} alignItems="center">
                                            <Grid item xs={12} md={6}>
                                                <TextField
                                                    label="Instru├º├úo de Trabalho (Ajuda)" size="small" fullWidth
                                            ...
                                            <Grid item xs={12} md={6}>
                                                <Stack direction="row" spacing={2} justifyContent={{ xs: 'flex-start', md: 'flex-end' }}>
                                                    <FormControlLabel control={<Switch size="small" checked={item.obrigatorio} onChange={e => handleUpdateItem(sIdx, iIdx, 'obrigatorio', e.target.checked)} />} label={<Typography variant="caption" fontWeight="500">Obrigat├│rio</Typography>} />
                                ...
                                <Button fullWidth variant="outlined" startIcon={<Plus size={16} />} onClick={() => handleAddItem(sIdx)} sx={{ borderStyle: 'dashed', height: 48, color: 'text.secondary' }}>Adicionar Pergunta ├á se├º├úo "{secao.titulo || 'Nova'}"</Button>
                            </Box>
                        </AccordionDetails>
                    </Accordion>
                ))}
